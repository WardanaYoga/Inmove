...
try:
    while True:
        ...
        if result.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(result.multi_hand_landmarks):
                ...
                # ===== SERIAL KANAN =====
                if hand_label == "Right":
                    if current_time - last_send_time_r > SEND_INTERVAL:
                        ...
                        last_send_time_r = current_time

                # ===== SERIAL KIRI =====
                else:
                    if current_time - last_send_time_l > SEND_INTERVAL:
                        diff = [abs(servo_send[i] - last_sent_l[i]) for i in range(5)]
                        if any(d > CHANGE_THRESHOLD for d in diff):
                            data = ','.join(map(str, servo_send)) + '\n'
                            arduino_left.write(data.encode())
                            last_sent_l = servo_send.copy()
                        last_send_time_l = current_time

                mp_draw.draw_landmarks(
                    frame, hand_landmarks, mp_hands.HAND_CONNECTIONS
                )

        cv2.imshow("Dual Hand Tracking FINAL", frame)
    ...
    arduino_right.close()
    arduino_left.close()
    print("Semua koneksi ditutup dengan aman.")
